...
export default function VendorDetailModal({
  ...
  const [reviews, setReviews] = useState([
    { id: 1, name: 'Chioma & Tunde K.', rating: 5, date: '2 weeks ago', comment: 'Absolutely phenomenal service! Delivered hot party Jollof on time in Lekki. Our guests loved every bite!' },
    { id: 2, name: 'Engr. Babatunde O.', rating: 5, date: '1 month ago', comment: 'CAC verification gave me peace of mind. Very professional team!' }
  ]);
  ...
  const handleLaunchWhatsApp = () => {
    ...
    const text = encodeURIComponent(
      `Hello ${vendor.name}!\n\nI found your verified business listing on OwambeHub Nigeria.\n\n` +
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/70 backdrop-blur-sm flex items-center justify-center p-3 sm:p-6 animate-fade-in">
      ...
      <div className="relative w-full max-w-4xl bg-white rounded-3xl shadow-2xl overflow-hidden my-auto max-h-[90vh] flex flex-col">
        ...
        <div className="sticky top-0 z-20 bg-white/90 backdrop-blur-md px-6 py-4 border-b border-slate-200 flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            {vendor.verified && (
              <span className="flex items-center space-x-1 px-2.5 py-1 rounded-full bg-emerald-100 text-emerald-800 font-bold text-xs">
        ...
        <div className="overflow-y-auto p-6 space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="flex items-center space-x-2 mt-1.5 text-xs font-semibold text-slate-500">
              ...
              <div className="flex flex-wrap items-center gap-3">
                <div className="flex items-center space-x-1.5 px-3 py-1 bg-amber-50 border border-amber-200 text-amber-900 rounded-xl text-xs font-bold">
                ...
                {vendor.cacRegistered && (
                  <div className="flex items-center space-x-1 px-3 py-1 bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-xl text-xs font-bold">
              ...
              <div className="flex items-center space-x-4 pt-2 border-t border-slate-100 text-xs font-semibold text-slate-600">
                <div className="flex items-center space-x-1">
                  <Instagram className="w-4 h-4 text-pink-600" />
                  <span>{vendor.instagram || '@owambevendor'}</span>
                </div>
                <div className="flex items-center space-x-1">
              ...
              <div className="p-4 rounded-2xl bg-gradient-to-r from-emerald-900 to-slate-900 text-white flex items-center justify-between">
                <div>
                  <span className="text-[10px] uppercase font-bold text-amber-300 tracking-wider">Starting Package</span>
                ...
                <button
                  ...
                  className="px-4 py-2.5 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-white text-xs font-bold flex items-center space-x-2 shadow-md transition-all"
                >
                  ...
                  return (
                    <div
                      ...
                      className={`p-4 rounded-2xl border transition-all cursor-pointer flex flex-col justify-between ${
                        isSelected 
                          ? 'bg-amber-50/70 border-amber-500 shadow-md ring-2 ring-amber-400' 
                    ...
                    >
                      <div>
                        ...
                        <div className="text-base font-extrabold text-emerald-800 mb-3">
                          {pkg.price}
                        </div>
          ...
          <div className="bg-slate-50 p-6 rounded-3xl border border-slate-200 space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-base font-bold text-slate-900">Request Instant Event Quote</h3>
                <p className="text-xs text-slate-500">Send your event requirements directly to {vendor.name}</p>
              </div>
            ...
            <form onSubmit={handleQuoteSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  type="text"
                  required
                  placeholder="e.g., Mrs. Folake Adeniyi"
                  value={clientName}
                  onChange={(e) => setClientName(e.target.value)}
                  className="w-full px-3 py-2 bg-white rounded-xl border border-slate-200 text-xs text-slate-800 focus:ring-2 focus:ring-amber-500"
                />
              </div>
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">Proposed Event Date</label>
                <input
                  type="date"
                  value={eventDate}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">Estimated Guests</label>
                <select
                  value={guestCount}
                ...
                >
                  ...
                  <option value="500+">500+ Owambe Crowd</option>
              ...
              <div className="sm:col-span-2 flex flex-col sm:flex-row items-center gap-3 pt-2">
                <button
                  ...
                  className="w-full sm:w-auto px-6 py-2.5 rounded-xl bg-slate-900 text-white font-bold text-xs hover:bg-slate-800 transition-all flex items-center justify-center space-x-2"
                >
                ...
                <button
                  type="button"
                  onClick={handleLaunchWhatsApp}
                  className="w-full sm:w-auto px-6 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs transition-all flex items-center justify-center space-x-2 shadow-md"
                >
                  <MessageSquare className="w-4 h-4" />
          ...
          <div className="space-y-4 pt-4 border-t border-slate-200">
            ...
            <form onSubmit={handleAddReview} className="p-4 bg-white rounded-2xl border border-slate-200 space-y-3">
              ...
              <textarea
                ...
                className="w-full p-3 bg-slate-50 rounded-xl border border-slate-200 text-xs text-slate-800 focus:ring-2 focus:ring-amber-500"
              />

              <button
                type="submit"
                className="px-4 py-2 rounded-xl bg-amber-600 text-white font-bold text-xs hover:bg-amber-700 transition-all"
              >
